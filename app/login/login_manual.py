    ...
    while True:
        try:
            ...
            if 0 <= index < len(accounts):
                ...
                return selected_account.get("username"), selected_account.get("password"), selected_account.get("recover_user")
            else:
...
async def analyze_page_with_bs4(page, element_type="unknown"):
    ...
    if element_type == "username_field":
        ...
        # Selector específico para X.com basado en data-testid del HTML
        specific_data_testid = "LoginForm_Login_Button"  # <--- Selector clave
        
        # Buscar botones por atributos prioritarios primero
        for element in soup.find_all(['button', 'div', 'a']):
            element_testid = element.get('data-testid', '')
            element_role = element.get('role', '')
            element_type_attr = element.get('type', '')
            element_text = element.get_text(strip=True).lower()
            
            # Priorizar data-testid exacto
            is_login_button = (
                element_testid == specific_data_testid  # <--- Coincidencia exacta
                or any(text in element_text for text in login_text_variations)
                or (element_role == 'button' and 'login' in element_text)
            )
            
            if is_login_button:
                selector = None
                
                # 1. Priorizar data-testid exacto
                if element_testid == specific_data_testid:
                    selector = f'[data-testid="{specific_data_testid}"]'  # <--- Selector directo
                
                # 2. Buscar por ID si existe
                elif element.get('id'):
                    selector = f'#{element["id"]}'
                
                # 3. Buscar por texto como último recurso
                else:
                    cleaned_text = element_text.replace(" ", "").replace("_", "")
                    for variation in login_text_variations:
                        if variation.replace(" ", "") in cleaned_text:
                            selector = f'button:has-text("{variation}")'
                            break
                
                if selector:
                    # Insertar el selector de data-testid al principio de la lista
                    if specific_data_testid in selector:
                        results["selectors"].insert(0, selector)  # <--- Prioridad máxima
                    else:
                        results["selectors"].append(selector)
                    
                    results["found"] = True
                    logger.debug(f"Login button selector added: {selector}")
...
async def handle_recovery_verification(page, recover_user, screenshot_dir):
    logger.info("Verificando pantalla de recuperación...")
    
    # Detectar campo usando data-testid del HTML proporcionado
    recovery_selector = 'input[name="text"][data-testid="ocfEnterTextTextInput"]'
    next_selector = 'button[data-testid="ocfEnterTextNextButton"]'
    
    # Esperar hasta 10 segundos por la pantalla
    recovery_detected = await wait_for_selector_or_continue(
        page, 
        recovery_selector, 
        timeout=10000,
        message="Buscando pantalla de verificación adicional..."
    )
    
    if recovery_detected and recover_user:
        logger.info("Pantalla de verificación detectada. Rellenando...")
        await type_human_like(page, recovery_selector, recover_user)
        await human_delay(1000, 2000)
        
        # Hacer clic en Next
        if await click_safely(page, next_selector):
            logger.info("Botón Next presionado en pantalla de verificación.")
            await page.screenshot(path=str(screenshot_dir / '5_recovery_screen.png'))
            await human_delay(3000, 5000)  # Esperar redirección
        else:
            logger.error("No se pudo hacer clic en el botón Next de recuperación")
    elif not recover_user:
        logger.warning("¡Cuenta no tiene recover_user! Manejar manualmente.")
...
async def manual_login():
    ...
    username, password, recover_user = select_account(accounts)
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if is_logged_in:
                ...
                # Manejar pantalla de verificación adicional (nuevo código)
                await handle_recovery_verification(page, recover_user, screenshot_dir)
