...
class ProfileActions:
    ...
    def __init__(self, page, config_path='app/config/profile_config.json'):
        """
    ...
    async def navigate_to_profile(self):
        ...
        try:
            ...
            # Comprobar si ya estamos en la página de perfil
            current_url = self.page.url
            if "twitter.com/" in current_url or "x.com/" in current_url:
                parts = current_url.split(".com/")
                if len(parts) > 1 and parts[1] and "/" not in parts[1]:
                    # Ya estamos en una página de perfil
                    self.logger.info(f"Ya estamos en la página de perfil: {current_url}")
                    return True
            
            # Intentar navegar directamente al perfil
            try:
                self.logger.info("Intentando navegar directamente al perfil")
                await self.page.goto("https://x.com/home")
                await asyncio.sleep(2)
            except Exception as e:
                self.logger.warning(f"Error al navegar directamente: {e}")
            
            # Hacer clic en el enlace de perfil
            self.logger.info("Buscando enlace de perfil")
            profile_link = await self._wait_for_selector_with_retry(self.selectors["profile_link"])
            if not profile_link:
                self.logger.error("No se encontró el enlace de perfil")
                return False
            
            self.logger.info("Haciendo clic en el enlace de perfil")
    ...
    async def edit_profile_single_modal(self, recover_user):
        ...
        try:
            ...
            # Navegar al perfil
            self.logger.info("Navegando al perfil")
            ...
            self.logger.info("Buscando botón de editar perfil")
            ...
            self.logger.info("Haciendo clic en el botón de editar perfil")
            ...
            if "name" in profile_data:
                self.logger.info("Actualizando campo de nombre")
            ...
            if "bio" in profile_data:
                self.logger.info("Actualizando campo de bio")
            ...
            if "location" in profile_data:
                self.logger.info("Actualizando campo de ubicación")
            ...
            if "profile_picture" in profile_data:
                ...
                if os.path.exists(img_path):
                    ...
                    if profile_pic_input:
                        # Subir imagen
                        self.logger.info("Subiendo imagen de perfil")
            ...
            # Guardar cambios
            self.logger.info("Buscando botón de guardar")
            ...
            self.logger.info("Haciendo clic en el botón de guardar")
            await self._human_delay()
    ...
    async def edit_profile_step_by_step(self, recover_user):
        ...
        try:
            ...
            # Navegar al perfil
            self.logger.info("Navegando al perfil (paso a paso)")
            if not await self.navigate_to_profile():
                return {"status": "error", "message": "Error al navegar al perfil"}
            
            # Hacer clic en el botón de configurar/editar perfil
            self.logger.info("Buscando botón de configurar/editar perfil")
            ...
            self.logger.info("Haciendo clic en el botón de configurar/editar perfil")
            ...
            if "profile_picture" in profile_data:
                ...
                self.logger.info("Verificando etapa de foto de perfil")
                ...
                if profile_pic_header:
                    self.logger.info("En etapa de foto de perfil")
                    img_path = os.path.join("assets", recover_user, profile_data["profile_picture"])
                    if os.path.exists(img_path):
                        # Esperar por el input de subida de archivo
                    ...
                    self.logger.info("Buscando botón siguiente")
                    next_button = await self._wait_for_selector_with_retry(self.selectors["photo_next_button"])
                    if next_button:
                        self.logger.info("Haciendo clic en botón siguiente")
            ...
            self.logger.info("Buscando botón de guardar final")
            final_save = await self._wait_for_selector_with_retry(self.selectors["final_save_button"], timeout=5000)
            if final_save:
                self.logger.info("Haciendo clic en botón de guardar final")
